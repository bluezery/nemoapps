...
static void _card_icon_color_timeout(struct nemotimer *timer, void *userdata)
{
...
static void _card_icon_move_timeout(struct nemotimer *timer, void *userdata)
{
    CardIcon *icon = userdata;
...
static void _card_icon_timeout(struct nemotimer *timer, void *userdata)
{
    CardIcon *icon = userdata;
...
CardIcon *card_create_icon(CardBackground *bg, const char *uri, double rx, double ry)
{
    ...
    icon->timer = TOOL_ADD_TIMER(bg->tool, 0, _card_icon_timeout, icon);
    icon->move_timer = TOOL_ADD_TIMER(bg->tool, 0, _card_icon_move_timeout, icon);
    icon->color_timer = TOOL_ADD_TIMER(bg->tool, 0, _card_icon_color_timeout, icon);
...
void card_icon_revoke(CardIcon *icon)
{
...
void card_icon_show(CardIcon *icon, uint32_t easetype, int duration, int delay)
{
    if (duration > 0) {
...
void card_icon_hide(CardIcon *icon, uint32_t easetype, int duration, int delay)
{
    if (duration > 0) {
        _nemoshow_item_motion(icon->group, easetype, duration, delay,
                "alpha", 0.0,
...
void card_icon_rotate(CardIcon *icon, uint32_t easetype, int duration, int delay, double ro)
{
    if (duration > 0) {
        _nemoshow_item_motion(icon->group, easetype, duration, delay,
                "ro", ro,
...
void card_icon_scale(CardIcon *icon, uint32_t easetype, int duration, int delay, double sx, double sy)
{
    if (duration > 0) {
        _nemoshow_item_motion(icon->group, easetype, duration, delay,
                "sx", sx, "sy", sy,
...
void card_icon_translate(CardIcon *icon, uint32_t easetype, int duration, int delay, double tx, double ty)
{
    if (duration > 0) {
        _nemoshow_item_motion(icon->group, easetype, duration, delay,
                "tx", tx, "ty", ty,
...
void card_icon_get_center(CardIcon *icon, void *event, double *cx, double *cy)
{
    RET_IF(!icon);
...
void card_icon_rotate_init(CardIcon *icon, NemoWidget *widget, void *event)
{
...
void card_icon_do_rotate(CardIcon *icon, NemoWidget *widget, void *event)
{
    if (list_count(icon->grabs) < 2) return;
    ...
    card_icon_rotate(icon, 0, 0, 0, icon->grab_ro - r1 * 180/M_PI);
}

void card_icon_do_scale(CardIcon *icon, NemoWidget *widget, void *event)
{
    if (list_count(icon->grabs) < 2) return;

    double cx, cy;
    card_icon_get_center(icon, event, &cx, &cy);
...
void card_icon_do_move(CardIcon *icon, NemoWidget *widget, void *event)
{
    double cx, cy;
    card_icon_get_center(icon, event, &cx, &cy);
    ...
    card_icon_translate(icon, 0, 0, 0,
            cx + icon->grab_diff_x,
            cy + icon->grab_diff_y);
}

void card_icon_do_move_up(CardIcon *icon, NemoWidget *widget, void *event)
{
    CardBackground *bg = icon->bg;
        ...
        if (k < 5) {
            ...
            if (abs(nemoshow_item_get_translate_x(icon->group) - tx) > bg->icon_throw_min_dist ||
                ...
                _nemoshow_item_motion(icon->group, NEMOEASE_CUBIC_OUT_TYPE, bg->icon_throw_duration, 0,
                        "tx", tx, "ty", ty, NULL);
                if (WELLRNG512()%2 == 0) {
                    nemosound_play(CARD_SOUND_DIR"/boom.wav");
                } else {
                    nemosound_play(CARD_SOUND_DIR"/throwing.wav");
                }
            }
...
void card_icon_move_init(CardIcon *icon, NemoWidget *widget, void *event)
{
    // diff update
    double cx, cy;
    card_icon_get_center(icon, event, &cx, &cy);
...
void card_icon_append_grab(CardIcon *icon, NemoWidget *widget, void *event)
{
    Grab *g = grab_create(nemoshow_event_get_device(event));
    icon->grabs = list_append(icon->grabs, g);

    card_icon_move_init(icon, widget, event);
    card_icon_rotate_init(icon, widget, event);
    card_icon_scale_init(icon, widget, event);
}

void card_icon_remove_grab(CardIcon *icon, NemoWidget *widget, void *event)
{
    List *l;
    Grab *g;
    LIST_FOR_EACH(icon->grabs, l, g) {
    ...
    card_icon_rotate_init(icon, widget, event);
    card_icon_scale_init(icon, widget, event);
    card_icon_do_move_up(icon, widget, event);
}

static void _card_icon_grab_event(NemoWidgetGrab *grab, NemoWidget *widget, struct showevent *event, void *userdata)
{
    CardIcon *icon = userdata;
    CardBackground *bg = icon->bg;
    ...
    if (nemoshow_event_is_down(show, event)) {
        card_icon_append_grab(icon, widget, event);
        if (list_count(icon->grabs) == 1) {
        ...
        if (g->device == nemoshow_event_get_device(event)) {
            card_icon_do_move(icon, widget, event);
            card_icon_do_scale(icon, widget, event);
            card_icon_do_rotate(icon, widget, event);
        }
    } else if (nemoshow_event_is_up(show, event)) {
        card_icon_remove_grab(icon, widget, event);
    }
    nemoshow_dispatch_frame(bg->show);
}

static void _card_background_event(NemoWidget *widget, const char *id, void *info, void *userdata)
{
...
static void _card_background_gallery_timeout(struct nemotimer *timer, void *userdata)
{
    CardBackground *bg = userdata;
...
CardBackground *card_background_create(NemoWidget *parent, int width, int height, const char *bgpath, int gallery_timeout, int gallery_duration, int sketch_timeout, int sketch_min_dist, int sketch_dot_cnt, int icon_cnt, int icon_history_cnt, int icon_history_min_dist, int icon_throw_min_dist, int icon_throw_coeff, int icon_throw_duration, List *icons)
{
    ...
    bg->gallery_timer = TOOL_ADD_TIMER(bg->tool, 0, _card_background_gallery_timeout, bg);
    ...
    nemowidget_append_callback(widget, "event", _card_background_event, bg);
    ...
    for (i = 0 ; i < icon_cnt ; i++) {
        ...
        CardIcon *icon;
        icon = card_create_icon(bg, path, rx, ry);
        card_icon_translate(icon, 0, 0, 0, tx, ty);
...
void card_background_show(CardBackground *bg)
{
    ...
    LIST_FOR_EACH(bg->icons, l, icon) {
        card_icon_show(icon, NEMOEASE_CUBIC_OUT_TYPE, 1000, delay);
        delay += 250;
    }
...
void card_background_hide(CardBackground *bg)
{
    nemowidget_show(bg->widget, 0, 0, 0);
    ...
    CardIcon *icon;
    LIST_FOR_EACH(bg->icons, l, icon) {
        card_icon_revoke(icon);
        card_icon_hide(icon, NEMOEASE_CUBIC_IN_TYPE, 1000, 0);
    }
...
static void _card_background_win_layer(NemoWidget *win, const char *id, void *info, void *userdata)
{
    CardBackground *bg = userdata;
    int32_t visible = (intptr_t)info;
    if (visible == -1) {
        if (bg->visible) {
            card_background_hide(bg);
            bg->visible = false;
        }
    } else {
        if (!bg->visible) {
            card_background_show(bg);
            bg->visible = true;
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    CardBackground *bg = card_background_create(win, app->config->width, app->config->height,
    ...
    nemowidget_append_callback(win, "layer", _card_background_win_layer, bg);
    card_background_show(bg);
