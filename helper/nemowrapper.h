...
static inline void *_thread(void *userdata)
{
...
static inline Thread *thread_create(struct nemotool *tool, ThreadCallback callback, ThreadDoneCallback callback_done, void *userdata)
{
...
// Should be called main thread
static inline void worker_stop(Worker *worker)
{
...
static inline void *_worker_do(void *userdata)
{
...
static inline void _worker_done(bool canceled, void *userdata)
{
    Worker *worker = (Worker *)userdata;
    worker->thread = NULL;
...
// Should be called main thread
static inline void worker_start(Worker *worker)
{
    nemotimer_set_timeout(worker->timer, 100);
    if (worker->thread) {
        return;
...
// Should be called main thread
static inline Work *worker_append_work(Worker *worker, WorkCallback doo, WorkDoneCallback done, void *userdata)
{
    Work *work;
...
// Should be called main thread
static inline Worker *worker_create(struct nemotool *tool)
{
...
static inline void image_set_bitmap(Image *img, int width, int height, ImageBitmap *bitmap)
{
    if (img->bitmap) {
    ...
    image_bitmap_attach(img->bitmap, img->one);
}

static inline void _image_decode_done(bool cancel, void *userdata)
{
...
static inline void *_image_decode(void *userdata)
{
    ImageLoader *loader = (ImageLoader *)userdata;
...
static inline void *_image_decode_full(void *userdata)
{
    ImageLoader *loader = (ImageLoader *)userdata;
...
static inline void *_image_decode_fit(void *userdata)
{
    ImageLoader *loader = (ImageLoader *)userdata;
...
static inline ImageLoader *image_load(Image *img, struct nemotool *tool,
...
static inline ImageLoader *image_load_full(Image *img, struct nemotool *tool,
        const char *uri, int width, int height,
        ImageLoadDone done, void *userdata)
{
    ImageLoader *loader = (ImageLoader *)calloc(sizeof(ImageLoader), 1);
...
static inline ImageLoader *image_load_fit(Image *img, struct nemotool *tool,
        const char *uri, int width, int height,
        ImageLoadDone done, void *userdata)
{
    ImageLoader *loader = (ImageLoader *)calloc(sizeof(ImageLoader), 1);
...
static inline void imageloader_cancel(ImageLoader *loader)
{
    thread_destroy(loader->thread);
}

static inline Image *image_create(struct showone *parent)
{
...
static inline void image_load_cancel(Image *img)
{
    if (img->loader) {
...
static inline void image_unload(Image *img)
{
    image_load_cancel(img);
    if (img->bitmap) {
        image_bitmap_detach(img->one);
        image_bitmap_destroy(img->bitmap);
        img->bitmap = NULL;
    }
}

static inline void image_destroy(Image *img)
{
...
static inline int image_get_width(Image *img)
{
    return nemoshow_item_get_width(img->one);
}

static inline int image_get_height(Image *img)
{
    return nemoshow_item_get_height(img->one);
}

static inline void image_load_sync(Image *img, const char *uri, int width, int height)
{
    if (!file_is_exist(uri)) {
        ERR("file is not existed: %s", uri);
        return;
...
static inline void image_translate(Image *img, uint32_t easetype, int duration, int delay, double tx, double ty)
{
    if (duration > 0) {
...
static inline void image_scale(Image *img, uint32_t easetype, int duration, int delay, double sx, double sy)
{
    if (duration > 0) {
        _nemoshow_item_motion(img->group, easetype, duration, delay,
                "sx", sx, "sy", sy, NULL);
...
static inline void image_rotate(Image *img, uint32_t easetype, int duration, int delay, double ro)
{
    if (duration > 0) {
        _nemoshow_item_motion(img->group, easetype, duration, delay,
                "ro", ro, NULL);
...
static inline void image_set_alpha(Image *img, uint32_t easetype, int duration, int delay, double alpha)
{
    if (duration > 0) {
        _nemoshow_item_motion(img->group, easetype, duration, delay,
                "alpha", alpha, NULL);
...
static inline void image_set_anchor(Image *img, double ax, double ay)
{
    nemoshow_item_set_anchor(img->one, ax, ay);
}


static inline void image_set_clip(Image *img, struct showone *clip)
{
    img->clip = clip;
