...
static char* name_getter(uint32_t index, void *data)
{
	char buffer[32];
	struct realdata *d = (struct realdata *) data;
	sprintf(buffer, "%s %d", d->name, d->count);
...
static char* diameter_getter(uint32_t index, void *data)
{
	char *w;
	...
	start = start - 20 * index;
	sprintf(buffer, "%d", start);
	return strdup(buffer);
	*/
}

static char* inner_getter1(uint32_t index, void *data)
{
	char *inner;
...
static char* inner_getter2(uint32_t index, void *data)
{
	char *inner;
...
static char* color_getter2(uint32_t index, void *data)
{
	char buffer[32];
	struct realdata *d = (struct realdata *) data;
	sprintf(buffer, "%u", d->color);
...
static char* value_size_getter(uint32_t index, void *data)
{
	char *size;
...
static char* name_size_getter(uint32_t index, void *data)
{
	char *size;
...
static char* stroke_width_getter(uint32_t index, void *data)
{
	char *sw;
...
static void create_chart(struct showone* canvas, struct context *ctx)
{
	...
	layout = nemodavi_layout_append(davi, NEMODAVI_LAYOUT_DONUT);
	...
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR1, diameter_getter);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR2, inner_getter1);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR3, inner_getter2);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR4, gap_getter1);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR5, color_getter2);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR6, value_size_getter);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR7, name_size_getter);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR8, stroke_width_getter);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR9, gap_getter2);
