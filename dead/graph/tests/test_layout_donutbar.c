...
static char* strokew_getter(uint32_t index, void *data)
{
	char *strokew;

	strokew = "33.0";

	printf("%s: %s\n", __FUNCTION__, strokew);
	return strokew;
}

static char* gap_getter(uint32_t index, void *data)
{
	char *gap;
...
static void create_chart(struct showone* canvas, struct context *ctx)
{
	...
	layout = nemodavi_layout_append(davi, NEMODAVI_LAYOUT_DONUTBAR);
	...
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR1, strokew_getter);
	nemodavi_layout_set_getter(layout, NEMODAVI_LAYOUT_GETTER_USR2, gap_getter);
